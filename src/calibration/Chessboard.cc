...
#include "calibration/ChessboardQuad.h"
#include "calibration/Spline.h"
...
void Chessboard::findCorners(bool useOpenCV)
{
    mCornersFound = findChessboardCorners(
        mImage, mBoardSize, mCorners, CV_CALIB_CB_ADAPTIVE_THRESH + CV_CALIB_CB_NORMALIZE_IMAGE +
    ...
    for (int k = 0; k < 6; ++k) {
        for (int dilations = minDilations; dilations <= maxDilations; ++dilations) {
            ...
            if (flags & CV_CALIB_CB_ADAPTIVE_THRESH) {
                ...
                // convert to binary
                cv::adaptiveThreshold(img, thresh_img, 255, CV_ADAPTIVE_THRESH_MEAN_C,
                                      CV_THRESH_BINARY, blockSize, (k / 2) * 5);
            } else {
...
// belong.
void Chessboard::cleanFoundConnectedQuads(std::vector<ChessboardQuadPtr>& quadGroup,
                                          cv::Size patternSize)
{
...
void Chessboard::findConnectedQuads(std::vector<ChessboardQuadPtr>& quads,
                                    std::vector<ChessboardQuadPtr>& group, int group_idx,
                                    int dilation)
{
    ChessboardQuadPtr q;
...
void Chessboard::labelQuadGroup(std::vector<ChessboardQuadPtr>& quadGroup, cv::Size patternSize,
                                bool firstRun)
{
...
int Chessboard::augmentBestRun(std::vector<ChessboardQuadPtr>& candidateQuads,
                               int candidateDilation, std::vector<ChessboardQuadPtr>& existingQuads,
...
//===========================================================================
void Chessboard::generateQuads(std::vector<ChessboardQuadPtr>& quads, cv::Mat& image, int flags,
                               int dilation, bool firstRun)
{
    ...
    for (size_t i = 0; i < contours.size(); ++i) {
        ...
        for (int approx_level = min_approx_level; approx_level <= max_approx_level;
             approx_level++) {
                                ...
                                std::vector<ChessboardCornerPtr>& corners, cv::Size patternSize)
{
    ...
    for (size_t i = 0; i < quads.size(); ++i) {
        ...
        for (int j = 0; j < 4; ++j) {
            ...
            if (c->column == max_col && c->row != min_row && c->row != max_row &&
                !c->needsNeighbor) {
                flagColumn = true;
            }
            if (c->row == max_row && c->column != min_col && c->column != max_col &&
                !c->needsNeighbor) {
                flagRow = true;
            }
        }
...
void Chessboard::getQuadrangleHypotheses(const std::vector<std::vector<cv::Point>>& contours,
...
void countClasses(const std::vector<std::pair<float, int>>& pairs, size_t idx1, size_t idx2,
...
bool Chessboard::matchCorners(ChessboardQuadPtr& quad1, int corner1, ChessboardQuadPtr& quad2,
                              int corner2) const
{
